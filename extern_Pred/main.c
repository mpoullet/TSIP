                 ...
                 int farbraum, BOOL tabular, char* psImageFile)
{
  ...
  if (reconstruction == FALSE)
  {
    ...
    if (tabular == FALSE)
    {
      printf( "\nEntropy     Source (%6s) Output (%6s) %s\n", 
              strFarbraumOut, strFarbraumOut,
              p->blockMode == TRUE ? 
              "S[Bits]  Expected Bitrate (Image with SInfo)" : "");
      for ( i = 0; i < in->channelCnt; i++)
      {
        float outH, outS;
        
        outH = entropie( out->img[i]);
        outS = sInfoEntropie( out->sInfo[i]) * (float)out->sInfo[i]->length;

        printf( "Channel #%d:   %11.3f ->  %11.3f", i + 1,
                entropie( in->img[i]), outH);
        if (p->blockMode == TRUE)
          printf( "  %7.0f   %11.3f", ceilf(outS), 
                  outH + outS / out->img[i]->length);

        printf( "\n");
      }

      /* print predictor selection */
      if (p->blockMode == TRUE || p->wMeanMode == FALSE)
      {
        UINT *pCount, j;
        char **pName;
        ...
        for ( i = 0; i < out->channelCnt; i++)
          for ( r = 0; r < out->psImg[i]->height; r++)
            for ( c = 0; c < out->psImg[i]->width; c++)
              pCount[ (out->psImg[i]->wert[r][c] * PREDICTORCOUNT)
                      / out->psImg[i]->maxValue]++;
...
Optional parameters [options]:                                    \n\
 ...
 -bc<#>     block comparison mode [0: Entropy (default), 1: SSE]  \n\
 ...
 -ts<#>     sigma value for template blending                     \n\
Further information:                                              \n\
...
int main( int argc, char *argv[])
{
    ...
    switch ( argv[1][1]) /* get option */
      ...
      case 'b': /* parameters related to the block module */
            ...
            p->blockHeight = atoi( &argv[1][3]);
						break;
          case 'c': /* set selection criterion */
            p->selCriterion = atoi( &argv[1][3]);
            break;
				}
				break;
  ...
  else if (dbFile == NULL && p->wMeanMode == FALSE)
    fprintf( stderr, 
      "\nA context data file has to be specified for context-based predictor selection!\n");
  else
  {
