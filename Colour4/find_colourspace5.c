 ...
 * 17.08.2016 copy from find_colourspace4.c, use energy instead of entropy as criterion
 *****************************************************************/
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
										 ...
										 )
{
	...
	/* for prediction mode use energy	*/
	double EnY[NUM_Y];	/* energy in Y component	*/
	double EnUV[NUM_UV];			/* energy in U or V component	*/
	double MeanY[NUM_Y];	/* mean in Y component	*/
	double MeanUV[NUM_UV];			/* mean in U or V component	*/
	double min_Energy, energ, energy[4]; 
...
//step_size = 1;
			...
			for (x = x_start, pos = py+x_start; x < x_end, x < im->width; x++, pos++)
			{
				...
				if (pred_flag)
				{
					...
					for ( i = 0; i < NUM_Y; i++)	/* all Y components	*/
					{
						...
						diff = YUV_line[i][0][x] - xHat;
						EnY[i] += diff * diff;		/* use energy criterion	*/
						MeanY[i] += diff;		/* use variance criterion	*/
					}
					...
					for ( i = NUM_Y; i < NUM_Y+UV_COMPO; i++)	/* all UV components	*/
					{
						...
						diff = YUV_line[i][0][x] - xHat;
						EnUV[i-NUM_Y] += diff * diff;		/* use energy criterion	*/
						MeanUV[i-NUM_Y] += diff;		/* use variance criterion	*/
					} /* for i=9	*/
				}
				else /* do not use prediction	*/
				{
					...
					for ( i = 0; i < NUM_Y; i++)	/* all Y components	*/
					{
						...
						HY[i][ YUV_line[i][0][x] + 511 ]++;	/*  histogram for entropy calculation	*/
						//EnY[i] += YUV_line[i][0][x] * YUV_line[i][0][x];
					}
					...
					for ( i = NUM_Y; i < NUM_Y+UV_COMPO; i++)	/* all UV components	*/
					{
						assert( YUV_line[i][0][x] + 511 >= 0);
						assert( YUV_line[i][0][x] + 511 < 1024);
						HUV[i-NUM_Y][ YUV_line[i][0][x] + 511 ]++; /* histogram for entropy calculation	*/
						//EnUV[i-NUM_Y] += YUV_line[i][0][x] * YUV_line[i][0][x];
					}
				}
			}	/* for x	*/
				...
				if (counter == 0)	/* now count the prediction error	*/
				{
					...
					else
					{
						for ( i = 0; i < NUM_Y; i++)	/* all Y components	*/
						{
							assert( YUV_line[i][0][x] + 511 >= 0);
							assert( YUV_line[i][0][x] + 511 < 1024);
							HY[i][ YUV_line[i][0][x] + 511]++;	/* use entropy criterion	*/
							//EnY[i] += YUV_line[i][0][x] * YUV_line[i][0][x];
						}
						for ( i = NUM_Y; i < NUM_Y+UV_COMPO; i++)	/* all UV components	*/
						{
							assert( YUV_line[i][0][x] + 511 >= 0);
							assert( YUV_line[i][0][x] + 511 < 1024);
							HUV[i-NUM_Y][ YUV_line[i][0][x] + 511]++; 	/* use entropy criterion	*/
							//EnUV[i-NUM_Y] += YUV_line[i][0][x] * YUV_line[i][0][x];
						}
					}
				} /*	if (counter == 0)	*/
	...
	if (pred_flag)
	{
		/* normalise energies	and convert to variances */
		for ( i = 0; i < NUM_Y; i++)	/* all Y components	*/
		{
			EnY[i] = EnY[i] / N;
			MeanY[i] = MeanY[i] / N;
			EnY[i] = EnY[i] - MeanY[i] * MeanY[i];
		}
		for ( i = 0; i < NUM_UV; i++)	/* all UV components	*/
		{
			EnUV[i] = EnUV[i] / N;
			MeanUV[i] = MeanUV[i] / N;
			EnUV[i] = EnUV[i] - MeanUV[i] * MeanUV[i];
		}

		min_Energy = EnY[0]; /* start with first value	*/
		min_Y_idx = 0;
		for ( i = 1; i < NUM_Y; i++)
		{
			if (min_Energy > EnY[i] && i < 9) 
			{
				min_Energy = EnY[i];
				min_Y_idx = i;
			}
		}
		energy[0] = EnY[min_Y_idx];

		/* get minimum energy	*/
		min_Energy = EnUV[0] + EnUV[1]; /* R-G	G-B	*/

		energy[1] = EnUV[0];
		energy[2] = EnUV[1];
		min_UV_idx = 0;

		energ = EnUV[0] + EnUV[2]; /* R-G	R-B	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 1;
		...
		energ = EnUV[0] + EnUV[3]; /* R-G		B-(R+3G)/4	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 3;
		...
		energ = EnUV[0] + EnUV[4]; /* R-G		B-(3R+G)/4		*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 4;
		...
		energ = EnUV[2] + EnUV[5]; /* R-B	G-(R+3B)/4 */
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 5;
		...
		energ = EnUV[1] + EnUV[6]; /* B-G	R-(B+3G)/4	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 6;
		...
		energ = EnUV[1] + EnUV[7]; /* B-G	R-(3B+G)/4	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 7;
		...
		energ = EnUV[2] + EnUV[8]; /* R-B	G-(B+3R)/4	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 8;
		...
		energ = EnUV[0] + EnUV[9]; /* R-G	B-(R+G)/2	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 9;
		...
		energ = EnUV[2] + EnUV[10]; /* R-B	G-(R+B)/2	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 10;
		...
		energ = EnUV[1] + EnUV[11]; /* B-G	R-(B+G)/2	*/
		if (energ < min_Energy)
		{
			min_Energy = energ;
			min_UV_idx = 11;
		...
		*UVj = min_UV_idx;
		...
		min_Energy += EnY[min_Y_idx]; /* combine entropies from UV and Y	*/
		/*---------------------------------------------------*/

		/* check RGB	*/
		energ = EnY[0] + EnY[1] + EnY[2];
		if (energ <= min_Energy)	/* preference to RGB if energy is identical	*/
		{
			min_Energy = energ;
			ct_type = RGBc;
			*Yi = 9; *UVj = 11;
		...
		/* check  R<=>G:  G-R R B	*/
		energ = EnUV[0] + EnY[1] + EnY[2];
		if (energ < min_Energy-0.015) /* penalty	*/
		{
			min_Energy = energ;
			ct_type = B2_1;
			*Yi = 9; *UVj = 2;
