...
set res_en="res_HXC_Science.txt"
set res_de="res_HXC_Science_d.txt"
...
for /f "usebackq delims=" %%I in (HXC_images_TestSet.txt) do (
set image=%%L
...
REM  pngtopnm.exe %%I > tmp.ppm
 ...
 %EXE% -i %%I -o tmp1.tsip -method HXC -Idx 1 -pred none >> %res_en% 
...
for /L %%C in (1 1 3) do (
 ...
 %EXE% -i %%I -o tmp4.tsip -method HXC -HXCincreasefac %%C -HXCauto 0 -Idx 1 -pred none >> %res_en% 
