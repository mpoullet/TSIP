...
set PSNR="PSNR_C.exe"
set logfile="log_LMSPRED_comp.txt"
set OutFile=tmpLMScomp.tsip
set res="res_LMScomp.txt"
set res_de="res_LMScomp_de.txt"
...
REM		for /L %%P in (6,6,18) do (
		%EXE% -i %%I  -o %OutFile% -searchSpace 19 -numOfPreds 18 -method TSIP -pred lmspred -colpred 0 -tilewidth 9999999 -tileheight 999999 -log_name %logfile% >> %res%
REM			)
REM	)
 %EXE% -i %OutFile% -o reco.pgm  >> %res_de%
 %PSNR% -i1 %%I -i2 reco.pgm  -p  >> %res%
