...
set EXE="..\bin\TSIPcoder_2_17.exe"
REM set EXE="..\bin\TSIPcoder_2_16b.exe"
set PSNR="PSNR_C.exe"
set logfile="log_CoBaLP2_auto_kodimGrey.txt"
set OutFile=tmpAuto_kodim.tsip
set res="res_CoBaLP2_autoColour_kodim.txt"
set res_de="res_CoBaLP2j_de.txt"
set DIR="..\..\..\Daten\"
...
REM ##################### Main Loop ##############################
REM	%DIR%\kodim??x.pgm roud_grey.png test2x.ppm
for  %%I in (                               
 	%DIR%\kodim??.pgm roud_grey.png test2.ppm
)do (
...
REM start with this images
 ...
 echo   %%I   >> %res%
  ...
  %EXE% -i %%I  -o %OutFile%  -method CoBaLP2 -sT 1 -sA 1 -sB 1 -aR 0 -aL 1 -idx 0 -constlines 0 -histmode comp -tilewidth 99999 -tileheight 99999 -log_name %logfile% >> %res%
REM
 %EXE% -i %OutFile% -o reco.pnm  >> %res_de%
REM
 %PSNR% -i1 %%I -i2 reco.pnm  -p  >> %res%
