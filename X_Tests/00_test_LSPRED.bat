...
set PSNR="PSNR_C.exe"
set logfile="log_LSPRED_ho_fast.txt"
set OutFile=tmpLS.tsip
set res="res_LS.txt"
set res_de="res_LS_de.txt"
...
REM		 
for  %%I in (      
	%DIR%\barbara.y.pgm %DIR%\kodim04.pgm	%DIR%\Ashton_landscape_grey.pgm %DIR%\squirrel.png 
)do (
...
REM  echo   %%I   >> %res%
	...
	for /L %%S in (19,2,19) do (
		for /L %%P in (18,6,18) do (
			echo SS %%S NP %%P


REM start with this images
			if %%I == %DIR%\kodim04.pgm (
				echo image ok
				if "%%S" == "13" (
					set flag=1
			...
			if !flag! == 1 (
				...
				echo .
				echo   %%I   >> %res%
				%EXE% -i %%I  -o %OutFile% -searchSpace %%S -numOfPreds %%P -method TSIP -pred lspredho -log_name %logfile% >> %res%
			)
		)
	)
REM    %EXE% -i %%I  -o %OutFile%   -log_name %logfile% >> %res%
