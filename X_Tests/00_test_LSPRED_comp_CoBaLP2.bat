...
set PSNR="PSNR_C.exe"
set logfile="log_LSPRED_comp.txt"
set OutFile=tmpLScomp.tsip
set res="res_LScomp.txt"
set res_de="res_LScomp_de.txt"
...
REM		for /L %%P in (6,6,18) do (
		%EXE% -i %%I  -o %OutFile% -searchSpace 19 -numOfPreds 18 -method TSIP -pred lspred -tilewidth 9999999 -tileheight 999999 -log_name %logfile% >> %res%
REM			)
REM	)
