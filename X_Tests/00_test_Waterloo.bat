...
set res_en="res_test_Waterloo.txt"
set res_de="res_test_Waterlood.txt"
...
REM ##################### Main Loop ##############################
for  %%I in (                               
      C:\Users\Strutz\Documents\Daten\uwaterloo.ca\*.tif       
) do (
...
REM %EXE% -i %%I -o tmp1.tsip -method BPC  -histMode none -Idx 0  1>> %res_en% 2>>&1
 ...
 %EXE% -i "%%I" -o tmpW4.tsip  -log_name log_TSIP_Waterloo.txt >> %res_en% 
REM %EXE% -i tmpW4.tsip -o recoW4.ppm  >> %res_de%
REM %PSNR% -i1 "%%I" -i2 recoW4.ppm  -p  >> %res_en%
