...
REM ##################### Main Loop ##############################
 for  %%I in (                               
	...
	C:\Users\Strutz\Documents\Daten\CT2\WikimediaCommons\LargeImages\*.png       
 ) do (

REM for /f "usebackq delims=" %%I in (HXC_images_WC.txt) do (
REM set image=%%L

echo %%I

REM start with this images
 if "%%I" == "C:\Users\Strutz\Documents\Daten\CT2\WikimediaCommons\PPM\LargeImages\George_Cruikshank_-_Tristram_Shandy,_Plate_VIII._The_Smoking_Batteries.ppm" (
 	set flag=1
  echo !flag!
 )
...
echo %%I
 ...
 %EXE% -i %%I -o tmp3L.tsip  -log_name log_TSIP_Wiki_Large.txt >> %res_en% 
