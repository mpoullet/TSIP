...
set PSNR="PSNR_C.exe"
set logfile="log_LSPRED_comp2.txt"
set OutFile=tmpLScomp2.tsip
set res="res_LScomp2.txt"
set res_de="res_LScomp2_de.txt"
...
REM start with this images
 if %%I == ..\..\..\Texte\Paper\2015-CoBaLP2_colour\Investigations\Testimages_YUV\leaves-6n3y_b01_Y.pgm (
 	set flag=1
  echo !flag!
 )
...
REM		for /L %%P in (6,6,18) do (
		%EXE% -i %%I  -o %OutFile% -searchSpace 20 -numOfPreds 18 -method TSIP -pred lspredho -tilewidth 9999999 -tileheight 999999 -log_name %logfile% >> %res%
REM			)
REM	)
